...
export default function SuperAdminMenu() {
  ...
  const addVariant = () =>
    setNewMenu({
      ...
      variants: [
        ...newMenu.variants,
        { variantName: "", variantType: "", quantityUnit: "", price: "", discountPrice: "", displayOrder: "", isDefault: false, isAvailable: true },
      ],
    });
  ...
  // ---------- AUTO-SELECT ADDONS & CUSTOMIZATION GROUPS ----------
  useEffect(() => {
    if (addons.length > 0 && newMenu.selectedAddons.length === 0) {
      setNewMenu((prev) => ({ ...prev, selectedAddons: addons }));
    }

    if (customizeGroups.length > 0 && newMenu.customizationGroupNames.length === 0) {
      setNewMenu((prev) => ({
        ...prev,
        customizationGroupNames: customizeGroups.map((g) => g.name || g.groupName),
      }));
    }
...
const handleSave = async (e) => {
  ...
  try {
    ...
    form.append("itemTypeId", itemType.id);
    ...
    form.append("foodTypeId", foodType.id);
    ...
    form.append("cuisineTypeId", cuisine.id); // proper ID for backend
    ...
    newMenu.variants.forEach((v, i) => {
      ...
      form.append(`variants[${i}].displayOrder`, v.displayOrder?.toString() || "1");
    });

    // ---------- Addons ----------
    newMenu.selectedAddons.forEach((a, i) => {
      form.append(`addons[${i}].addonName`, a.addOnName || a.name);
      form.append(`addons[${i}].isDefault`, (a.isDefault ?? false).toString());
      form.append(`addons[${i}].maxQuantity`, (a.maxQuantity ?? 1).toString());
    });
  ...
  // ---------- UI ----------
  return (
    <div className="Super-admin-menu-page">
      <ToastContainer />
      <h2 className="Super-admin-menu-title">
        <Newspaper size={20} /> Menu Management
      </h2>

      <div className="Super-admin-menu-controls">
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Search item..."
          className="Super-admin-menu-search"
        />
        <div>
          <button
            className="Super-admin-menu-add-btn"
            onClick={() => {
              resetForm();
              setShowModal(true);
            }}
                ...
                >
                  {menu.imageData ? (
                    <img
                      ...
                      alt={menu.itemName}
                      className="Super-admin-menu-img"
                      onError={(e) => {
                  ...
                  <h3 className="Super-admin-menu-name">{menu.itemName}</h3>
                  <p className="Super-admin-menu-type">{menu.itemType || "N/A"}</p>
                  <p className="Super-admin-menu-desc">{menu.description}</p>
                  <p className="Super-admin-menu-price">
                    ₹{" "}
      ...
      {showModal && (
        <div className="Super-admin-modal-overlay">
          <div className="Super-admin-modal">
            ...
            <form className="Super-admin-modal-body" onSubmit={handleSave}>
              ...
              <div className="Super-admin-grid-2">
                <div>
              ...
              {newMenu.image && <img src={newMenu.image} alt="preview" className="Super-admin-menu-preview" />}

              <h4>Variants</h4>
              {newMenu.variants.map((v, i) => (
                <div key={i} className="Super-admin-menu-variant-row">
                  <input
                    value={v.variantName}
                  ...
                  >
                    ...
                    <option value="PORTION">Portion(e.g., Half Plate, Full Plate, Family Pack)</option>
                    <option value="TEMPERATURE">TEMPERATURE(e.g., Hot, Cold, Iced)</option>
                    <option value="QUANTITY">Quantity(e.g., 250ml, 500ml, 1L, 2kg)</option>
                    <option value="PACKAGING">PACKAGING(e.g., Dine-in, Takeaway, Combo Pack)</option>
                    <option value="CUSTOM">CUSTOM(e.g., Chef’s Special, Seasonal Variant)</option>
                  </select>
                  <input
                    value={v.quantityUnit}
                    placeholder="Quantity Unit (e.g., 50g / 5ml / 3pcs / plate)"
                    onChange={(e) => {
                      const updated = [...newMenu.variants];
              ...
              <button
                type="button"
                className="Super-admin-menu-add-variant-btn"
                onClick={addVariant}
              >
                <Plus size={12} /> Add Variant
              </button>
              <div className="Super-admin-menu-grid-2">
                <label>
                  <input
                    type="checkbox"
              ...
              <h4>Addons</h4>
              <select
                className="Super-admin-addon-select"
                value=""
                onChange={(e) => {
              ...
              <div className="Super-admin-addon-section">
                <label>Selected Addons</label>
                <div className="Super-admin-addon-box">
                  {newMenu.selectedAddons.length > 0 ? (
                    <div className="Super-admin-addon-grid">
                      {newMenu.selectedAddons.map((a) => (
                        <span key={a.id || a.addOnId} className="Super-admin-addon-tag">
                          {a.addOnName || a.name}
                          <button
                            type="button"
                            className="Super-admin-addon-remove-btn"
                            onClick={() => {
                    ...
                    <p className="Super-admin-addon-empty">No addons selected</p>
                  )}
                </div>
              </div>
              ...
              <select
                className="Super-admin-customize-select"
                value=""
                onChange={(e) => {
              ...
              <div className="Super-admin-customize-section">
                <label>Selected Customization Groups</label>
                <div className="Super-admin-customize-box">
                  {newMenu.customizationGroupNames.length > 0 ? (
                    <div className="Super-admin-customize-grid">
                      {newMenu.customizationGroupNames.map((name) => (
                        <span key={name} className="Super-admin-customize-tag">
                          {name}
                          <button
                            type="button"
                            className="Super-admin-customize-remove-btn"
                            onClick={() => {
                    ...
                    <p className="Super-admin-customize-empty">No customization groups selected</p>
                  )}
                </div>
              </div>
              <div className="Super-admin-menu-modal-actions">
                <button type="submit" className="Super-admin-menu-save-btn">
                  Save Menu
                </button>
                <button
                  type="button"
                  className="Super-admin-menu-cancel-btn"
                  onClick={resetForm}
                >
                  Cancel
                </button>
